"""
CatBoost model training and evaluation utilities.
...
from utils.model_utils import apply_feature_scaling, save_model_artifacts, load_model_artifacts
...
class CatBoostTrainer:
    """
    Trainer class for CatBoost models with cross-validation support.
    ...
    def train_and_evaluate_internal(
        ...
        """
        Train and evaluate CatBoost models using internal cross-validation.
        ...
        for outcome_var in outcome_vars:
            ...
            tuning_dir = os.path.join(outcome_dir, 'catboost_tuning')
    ...
    def evaluate_external(
        ...
        """
        Evaluate pre-trained models on an external dataset.
        
        Uses models trained on one dataset to make predictions on a completely
        different dataset for external validation.
        ...
        for outcome_idx, outcome_var in enumerate(outcome_vars):
            ...
            trained_tuning_dir = os.path.join(trained_model_dir, outcome_var, 'catboost_tuning')
    ...
    def _train_and_evaluate_fold(
        ...
        from utils.model_utils import get_default_catboost_params
        ...
        if not os.path.exists(model_path):
            ...
            save_model_artifacts(model, scaler, best_params, model_prefix, 'catboost')
        else:
            model, scaler, _ = load_model_artifacts(model_prefix, 'catboost')
    ...
    def _evaluate_external_fold(
        ...
        if not os.path.exists(f'{model_prefix}_model.pkl'):
            raise FileNotFoundError(f"Trained model not found: {model_prefix}_model.pkl")
        ...
        y_scores = model.predict_proba(data_scaled[feature_cols])[:, 1]
